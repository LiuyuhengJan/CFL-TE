...
class EnvRuuner():
    ...
    def reset(self, time = -1):
        if time == -1:
            self.init_time = np.random.randint(0,20)*10000 #np.random.randint(0,3000)
            # with open('0803.txt', 'a') as file:
            #     file.write(f"全局开始 {self.init_time}\n")
            print('全局开始',self.init_time)
            # sys.stdout.close()
        ...
        # print('全局开始',self.init_time)
        self.gl_times = {}
        self.clock_time = self.init_time
        ...
        if self.args.iid == -2:
            ...
            for i in range(self.args.num_edges):
                ...
                self.gl_times[i] = self.init_time
                # edges[i].ini_loc()
        ...
        self.fl_done = False
        self.global_acc = 0
        self.edges_obs = self.get_sat_obs_nlink()
    ...
    def round_reset(self):
        avgtime = 10000 / 40
        x1 = 0.91/40
        reward_t = self.global_acc/x1 - self.round_time
        reward = self.round_ok_num * 5 + reward_t / avgtime

        if self.global_acc > 0.91:
            self.fl_train_time = self.clock_time - self.init_time
            self.fl_done = True
            # sys.stdout = open('output1.txt', 'a')
            # with open('0803txt', 'a') as file:
            #     file.write(f"{self.init_time} 一次结束 {self.fl_train_time} {self.selected_nodes} {self.global_acc}\n")
            print(self.init_time, '一次结束', self.fl_train_time, self.selected_nodes,self.global_acc)
        ...
        self.fl_train_time = self.clock_time - self.init_time
        if self.fl_train_time > 150000:
            self.fl_train_time = self.clock_time - self.init_time
            self.fl_done = True
            sys.stdout = open('output1.txt', 'a')
            # with open('output4.txt', 'a') as file:
            #     file.write(f"{self.init_time} 一次结束 {self.fl_train_time} {self.selected_nodes} {self.global_acc} 失败了\n")
            print(self.init_time, '一次结束', self.fl_train_time, self.selected_nodes,self.global_acc)

            # sys.stdout.close()

            if self.fl_train_time > 10000:
                reward = -((self.fl_train_time / avgtime) ** 0.5) * 20 + reward -100
            else:
                reward = (10000 - self.fl_train_time) / 10 + 100 + reward

        reward = round(reward, 2)
        print('ok_num:', self.ok_num)
    ...
    def step(self, action, done = False, h1 = False):
        ...
        while not done:
            ...
            if len(self.Con.FLs[0].receiver_buffer) + len(self.Con.FLs[0].no_receive) == len(self.selected_nodes):
                ...
                if all_acc > 0.91:
                    ...
                    # print('完成时间：',self.init_time,self.clock_time-self.init_time,all_acc)
                    #sys.stdout.close()
                    # print('完成时间：', self.ini_time, self.clock_time - self.ini_time, all_acc)
    ...
    #     obs = []
    #     obs.append("@4")  #
    #     obs.append(self.clock_time // 86400)
    #     obs.append("@4")
    #     obs.append("@2")  #
    #     obs.append((self.clock_time%86400)//7200)
    #     obs.append("@2")
    #     obs.append("@")
    #     obs.append(((self.clock_time%86400)%7200) // 200)
    #     obs.append("@")  # #+2
    #     obs.append('#')
    ...
    #     obs.append(round(self.global_acc,3))
    #     obs.append("@1")  # +2
