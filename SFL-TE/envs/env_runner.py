...
class EnvRuuner():
    ...
    def reset(self):
        ...
        from algorithm import test_h
        from envs import hhconfig

        all_args = hhconfig.get_parser().parse_known_args()[0]

        self.take_action = test_h.PPO(all_args)

        self.take_action.load_model('hlevel_model_0810_200.pth')
    ...
    def step(self):
        ...
        shibai = 0
        obss = self.get_sat_obs_nlink()
        action = self.take_action.take_test_action(obss)
    ...
    def get_sat_obs_nlink(self):
        ...
        obs.append("@2")
        obs.append("@")
        obs.append(((self.clock_time%86400)%7200) // 100)
        obs.append("@")  # #+2
        obs.append('#')
