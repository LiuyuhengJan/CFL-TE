...
# import uavsever
import envs.satsever as satsever
...
###############################################################################
# const class
with open('./envs/random_numbers.txt', 'r') as file:
...
class Constellation():
	...
	def __init__(
		...
		self.number_of_planes = Config.planes
		# print("6",self.number_of_planes)
		self.nodes_per_plane = Config.nodes_per_plane
		self.total_sats = Config.planes*Config.nodes_per_plane
		self.ground_node_counter = 0
		self.inclination = Config.inclination
		self.semi_major_axis = Config.semi_major_axis
		self.period = self.calculateOrbitPeriod(semi_major_axis=self.semi_major_axis)
		self.eccentricity = Config.ecc
		self.current_time = 0
		...
		self.G = None
		self.ids_sats = np.linspace(1,self.total_sats,self.total_sats)
		self.com_r = 0.3
		...
		self.raan_offsets = [(Config.arcOfAscendingNodes / self.number_of_planes)*i for i in
		...
		self.time_offsets = [(self.period/Config.nodes_per_plane)*i for i in
		                     range(0, Config.nodes_per_plane)]
	...
	def initSatelliteArray(self):
		...
		for plane in range(0, self.number_of_planes):
			for node in range(0, self.nodes_per_plane):
				...
				if(unique_id==5 or unique_id==12 or unique_id==18):
					self.FLs.append(satsever.SATF(unique_id))
				...
				self.satellites_array[unique_id]['pro_capacity'] = numbers[unique_id]
				# print('self.satellites_array[unique_id]',self.satellites_array[unique_id]['pro_capacity'])
	...
	def getArrayOfSatPositions(self):
		...
		# print("卫星location",len(sat_positions),sat_positions)
	...
	def getArrayOfGndPositions(self):
		...
		ground_positions = np.copy(self.groundpoints_array[['x', 'y', 'z']])
		# print("getArrayOfGndPositions",ground_positions)
	...
	def calculateIdealLinks(self, max_isl_range, max_stg_range):
		...
		if USING_NUMBA is True:
			...
			# print("calculateIdealLinks")

			link_idx = 0

			# add the ISL links  优化 要改 可能要改这个地方用距离判断是否有链接具体通信还要改一下
				...
				for sat_idx in range(self.total_sats):
					...
					if d < max_stg_range:
						if link_idx < self.link_array_size - 1:
							...
							self.link_array[link_idx]['distance'] = np.int32(d)
							# print("对比", self.current_time,self.link_array[link_idx])
							link_idx = link_idx + 1
	...
	def route_here(self, max_isl_range, max_stg_range):
		link_idx = 0
		# # add the ISL links  优化 要改 可能要改这个地方用距离判断是否有链接具体通信还要改一下
		# for sat_idx_a in range(self.total_sats - 1):
		# 	# copy the position of sat a
	...
	def judgment_link(self, max_stg_range):
...
#	@numba.jit(nopython=True)
	def numba_calculateIdealLinks(
		...
		# print("numba_calculateIdealLinks")
...
#	@numba.jit(nopython=True)
	...
	#搞一个链路状态的函数
	def S_S_link_state(self):

		return

	def G_S_link_state(self):
	...
	def random_node_transfer(self):
		num_nodes_to_transfer = random.randint(int(0.1*self.total_sats),int(self.com_r*self.total_sats))

		# 随机选择要传输的节点
		nodes_to_transfer = random.sample(self.ids_sats, num_nodes_to_transfer)

		# num_nodes_to_transfer = random.randint(int(0.1 * self.total_sats), int(self.com_r * self.total_sats))
		# 随机选择目标节点
		target_nodes = random.sample(self.ids_sats, num_nodes_to_transfer)

		for i in range(0,num_nodes_to_transfer):
			if(nodes_to_transfer != target_nodes):
				self.time_snapshot_shortest_path()

		return nodes_to_transfer, target_nodes

	def time_snapshot_shortest_path(self, graph, source, target, current_time):
		# 创建一个新的有向图，代表当前时间快照的网络状态
		current_snapshot_graph = nx.DiGraph()

		for u, v, data in graph.edges(data=True):
			if 'start_time' in data and 'end_time' in data:
				if data['start_time'] <= current_time <= data['end_time']:
					current_snapshot_graph.add_edge(u, v, weight=data['weight'])

		if source not in current_snapshot_graph.nodes() or target not in current_snapshot_graph.nodes():
			raise ValueError("源节点或目标节点不在当前时间快照中")

		# 使用Dijkstra算法查找最短路径
		shortest_path = nx.shortest_path(current_snapshot_graph, source=source, target=target, weight='weight')

		return shortest_path

	def u_s_t(self,uavl, timesc):
		self.setConstillationTime(timesc)
		self.getArrayOfSatPositions()
		for i in range(0,self.nodes_per_plane):
			for j in range(0, self.number_of_planes):
				a = pow(self.satellites_array[i*j+i]['x'] - uavl[0],2)
				b = pow(self.satellites_array[i*j+i]['y'] - uavl[1],2)
				c = pow(self.satellites_array[i*j+i]['z'] - uavl[2],2)
				if(pow((a+b+c),0.25)>909424):

					return False
				else:
					print("yes",uavl,"oo",pow((a+b+c),0.5))
					return True

	def UAVPoint(self, latitude, longitude, altitude):
	...
	def packet_error_calculator(distance_matrix, channel_interference, user_max_power):
		...
		q_packet_error = 1 - np.exp(
			-1.08 * np.divide(channel_interference + 1e-14, user_max_power * np.power(distance_matrix, (-2))))
...
# HiFlash_Communication-Efficient_Hierarchical_Federated_Learning_With_Adaptive_Staleness_Control_and_Heterogeneity-Aware_Client-Edge_Association (1)