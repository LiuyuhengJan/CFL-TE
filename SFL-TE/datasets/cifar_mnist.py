...
#
#     return data_loaders
def iid_esize_split(dataset, args, kwargs, is_shuffle):
    """
    split the dataset to users
    Return:
        dict of the data_loaders
    ...
    # if num_samples_per_client == -1, then use all samples
    if num_samples_per_client == -1:
        num_samples_per_client = int(total_samples / args.num_clients)

    # 检查总样本数是否足够分配
    if num_samples_per_client * args.num_clients > total_samples:
        raise ValueError("总样本数不足以分配给所有客户端。请减少每个客户端的样本数或减少客户端数量。")

    data_loaders = [0] * args.num_clients
    dict_users, all_idxs = {}, [i for i in range(total_samples)]

    for i in range(args.num_clients):
        # 打印all_idxs, num_samples_per_client的长度
        # print(f"Client {i}: len(all_idxs) = {len(all_idxs)}, num_samples_per_client = {num_samples_per_client}")

        if len(all_idxs) < num_samples_per_client:
            raise ValueError(f"Not enough samples left to allocate {num_samples_per_client} samples to client {i}")
...
def get_cifar10(dataset_root, args): # cifa10数据集下只能使用cnn_complex和resnet18模型
    is_cuda = args.cuda
    kwargs = {'num_workers': 1, 'pin_memory':True} if is_cuda else{}
    if args.model == 'cifar10': # 'cnn_complex':
        print("what")
